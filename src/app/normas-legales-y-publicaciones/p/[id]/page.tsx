...
export function generateStaticParams() {
  return OTHER_PUBLICATIONS.filter(
    (p) => p.href.startsWith("/normas-legales-y-publicaciones/p/"),
  ).map((p) => ({ id: p.id }));
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const pubs =
    (cms.publications as typeof OTHER_PUBLICATIONS) || OTHER_PUBLICATIONS;
  const item = pubs.find((p) => p.id === id) || getOtherPublication(id);
  if (!item) return { title: "Publicaciones" };
  return {
    title: `${item.title} | Publicaciones | Municipalidad de La Molina`,
        ...
        <section className="portal-page-hero">
          <div className="mx-auto max-w-7xl px-4">
            <Reveal variant="up">
              ...
              <h1 className="portal-page-title mt-3">{item.title}</h1>
              <p className="mt-2 text-sm text-molina-muted">{item.date}</p>
            ...
            {documents.length === 0 ? (
              <div className="rounded-2xl border border-dashed border-slate-300 bg-white px-6 py-12 text-center">
                <p className="text-sm font-semibold text-molina-deep">
                  Aún no hay archivos publicados aquí
                </p>
                <p className="mt-2 text-sm text-molina-muted">
                  Sube los PDF desde el panel de administración en la pestaña
                  Normas Legales.
                </p>
              </div>
            ) : (
